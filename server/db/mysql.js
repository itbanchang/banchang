...
// 4 workers × 50 = 200 total concurrent queries across the cluster
// (previous 120 × 4 = 480 was exhausting MySQL max_connections under peak load)
...
function _buildConfig(profile) {
  return {
    ...
    connectionLimit: 50, // Per worker (4 workers = 200 total) — was 150×4=600 (exceeded MySQL max_connections)
    maxIdle: 15, // Hold fewer idle connections; free them faster (was 60)
    idleTimeout: 60000, // Close idle after 60s (was 120s) — reduce resource holding
    queueLimit: 200,
    connectTimeout: 5000,
    charset: 'utf8mb4',
    timezone: '+07:00',
...
export async function switchServer(serverId) {
  if (!SERVER_PROFILES[serverId]) {
    throw new Error(
      `Unknown server: ${serverId}. Available: ${Object.keys(SERVER_PROFILES).join(', ')}`
    );
  }
  ...
  logger.info(`[MySQL] Switched to ${serverId} — READ-ONLY enforced`, {
    ...
    safety: [
      'SET SESSION TRANSACTION READ ONLY',
      'WRITE_PATTERNS regex block',
      'DANGEROUS_PATTERNS regex block',
    ],
  });
...
export function getQueryMetrics() {
  return {
    ...QUERY_METRICS,
    avg_query_ms:
      QUERY_METRICS.total > 0 ? Math.round(QUERY_METRICS._total_ms / QUERY_METRICS.total) : 0,
...
export async function getPool() {
  ...
  try {
    ...
    logger.info('MySQL Connected', {
      ...
      poolSize: MYSQL_CONFIG.connectionLimit,
    });
...
setInterval(() => {
  ...
  if (entriesToDelete.length > 0) {
    setImmediate(() => {
      ...
      logger.debug('Cache cleanup', {
        entries_purged: entriesToDelete.length,
        cache_size: _cacheEntryCount,
      });
    });
  }
  ...
  if (_cacheEntryCount > MAX_CACHE_SIZE) {
    const sortedByAge = Array.from(CACHE_METADATA.entries()).sort(
...
export async function dbQueryHeavy(key, ttlMins, sql, params = [], opts = {}) {
  const ttlMs = ttlMins * 60000;
  const now = Date.now();
  const heavyTimeout = opts.timeoutMs || 25000;
  ...
  if (DB_CACHE[key] && DB_CACHE[key].data) {
    if (!_revalidating.has(key)) {
      ...
      dbQuery(sql, params, { timeoutMs: heavyTimeout })
        .then(data => {
    ...
    logger.debug(`[STALE_REVALIDATE] ${key}`, { willRefresh: !_revalidating.has(key) });
    return DB_CACHE[key].data;
  }
  ...
  try {
    ...
    const data = await dbQuery(sql, params, { timeoutMs: heavyTimeout });
...
export async function dbQueryOneHeavy(key, ttlMins, sql, params = [], opts = {}) {
  const rows = await dbQueryHeavy(key, ttlMins, sql, params, opts);
  return rows?.[0] || null;
}
...
const WRITE_PATTERNS =
  /^\s*(UPDATE|DELETE|INSERT|REPLACE|CREATE|DROP|ALTER|RENAME|TRUNCATE|GRANT|REVOKE|LOAD|CALL|SET\s+GLOBAL|FLUSH|OPTIMIZE|REPAIR|LOCK|UNLOCK)\b/i;
...
export async function dbQuery(sql, params = [], opts = {}) {
  ...
  if (CIRCUIT_BREAKER.state === 'OPEN') {
    ...
    if (elapsed < CIRCUIT_BREAKER.resetTimeMs) {
      throw new Error(
        `⚡ CIRCUIT_BREAKER: Database queries paused (${Math.round((CIRCUIT_BREAKER.resetTimeMs - elapsed) / 1000)}s until retry)`
      );
    }
  ...
  try {
    ...
    if (wasKilled) {
      throw new Error(
        `⏱️ QUERY_TIMEOUT: SQL exceeded ${timeoutSec}s limit and was killed to protect Slave1`
      );
    }
    ...
    import('../ai/selfHeal.js')
      .then(({ handleError }) =>
        handleError(err, { source: 'dbQuery', query: sql?.substring(0, 80) })
      )
      .catch(() => {});
