...
router.get(
  ...
  cached('erAIv2', 120000, async () => {
    const [today, hourly, triage, week] = await Promise.allSettled([
      ...
      dbQuery(
        `SELECT HOUR(er_regist_time) as hr, COUNT(*) as cnt FROM er_regist WHERE er_regist_date >= DATE_SUB(CURDATE(), INTERVAL 14 DAY) GROUP BY hr ORDER BY hr`
      ),
      dbQuery(
        `SELECT er_emergency_type as level, COUNT(*) as cnt FROM er_regist WHERE er_regist_date >= DATE_SUB(CURDATE(), INTERVAL 7 DAY) GROUP BY er_emergency_type ORDER BY cnt DESC`
      ),
      dbQuery(
        `SELECT er_regist_date as dt, COUNT(*) as cnt FROM er_regist WHERE er_regist_date >= DATE_SUB(CURDATE(), INTERVAL 7 DAY) GROUP BY er_regist_date ORDER BY dt`
      ),
    ]);
...
router.get(
  ...
  cached('medrecAI', 600000, async () => {
    const [coding, missing, topDx, cmi, late] = await Promise.allSettled([
      dbQueryOne(
        `SELECT COUNT(*) as total, SUM(CASE WHEN COALESCE(adjrw, rw, 0) > 0 THEN 1 ELSE 0 END) as coded FROM an_stat WHERE dchdate >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)`
      ),
      dbQueryOne(
      ...
      dbQueryOne(
        `SELECT ROUND(AVG(COALESCE(adjrw, rw)), 2) as cmi, ROUND(SUM(COALESCE(adjrw, rw)), 2) as sum_rw FROM an_stat WHERE dchdate >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)`
      ),
      dbQueryOne(
    ...
    const recs = [
      `30-day Coding Status: ${coded}/${total} (${codedPct}%) · CMI ${cmiVal.toFixed(2)} · Sum RW ${sumRw.toFixed(2)}`,
    ];
    if (codedPct < 90)
      recs.push(
        `Coding Completeness ${codedPct}% — ต่ำกว่าเกณฑ์ 90% เร่งทีม Coder + Set deadline 7 วันหลัง D/C`
      );
    if (miss > 0) recs.push(`${miss} เคสยังไม่มี Principal Diagnosis — ตรวจสอบและบันทึกให้ครบ`);
    if (lateCoding > 0)
      recs.push(`${lateCoding} เคสค้าง Coding >14 วัน — เร่งดำเนินการเพื่อไม่กระทบรายได้`);
    if (cmiVal > 0 && cmiVal < 1.0)
      recs.push(
        `CMI ${cmiVal.toFixed(2)} — ต่ำกว่า 1.0 ทบทวน Specificity ของ Diagnosis (Under-coding risk)`
      );
    if (top[0])
      recs.push(`Top Diagnosis: ${top[0].icd10} ${top[0].name || ''} (${top[0].cnt} เคส)`);
