...
const DISABLE_AUTH = (process.env.DISABLE_AUTH === 'true' || process.env.NO_AUTH === 'true' || process.env.NODE_ENV !== 'production');
...
const ROLE_PERMISSIONS = {
    director: {
        modules: ['finance', 'ipd', 'opd', 'clinical', 'er', 'dental', 'thaimed', 'phystherapy', 'ncd', 'medrec', 'xray', 'pharmacy', 'lab', 'quality', 'ai', 'admin', 'audit', 'system'],
    ...
    clinical: {
        modules: ['ipd', 'opd', 'clinical', 'er', 'dental', 'thaimed', 'phystherapy', 'ncd', 'xray', 'pharmacy', 'lab', 'quality', 'ai'],
...
if (!REFRESH_TOKEN_SECRET) {
    console.error('❌ WARNING: REFRESH_TOKEN_SECRET not set. Using JWT_SECRET as fallback (less secure)');
}
...
export function generateToken(user, isRefresh = false) {
    ...
    return jwt.sign(
        { id: user.id, username: user.username, role: user.role, full_name: user.full_name, type: isRefresh ? 'refresh' : 'access' },
        secret,
        { expiresIn }
    );
}
