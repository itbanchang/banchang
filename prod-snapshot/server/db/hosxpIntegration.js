...
export async function getERTodayPatients() {
  // Phase C — Live ER = patients still IN the ER (finish_time IS NULL),
  // not "all patients who visited today". er_dch_type is unreliable at BCH.
  const rows = await dbQuery(`
    ...
    GREATEST(0, TIMESTAMPDIFF(MINUTE, e.enter_er_time, NOW())) as stay_minutes,
