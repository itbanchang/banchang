...
import erauditRoutes from '../routes/eraudit.js';
...
import mrAuditRoutes from '../routes/mrAudit.js';
...
import claudeMetricsRoutes from '../routes/claudeMetrics.js';
...
export function registerRoutes(app) {
  ...
  // Patient satisfaction survey — PUBLIC. POST /submit is the QR-code endpoint
  // (anonymous patients fill it on their phone). GET /summary returns aggregate
  // numbers only, safe to expose. Rate-limited per IP.
  ...
  app.use('/api/eraudit', auditMiddleware('patient_info'), authorize('eraudit'), erauditRoutes);
  ...
  app.use('/api/mr-audit', auditMiddleware('patient_info'), authorize('medrec'), mrAuditRoutes);
  ...
  app.use('/api/overview', auditMiddleware('operational'), authorize('finance'), overviewRoutes);
  ...
  // Phase H Sprint 1 Task 2.2 — Claude API cost monitoring (admin-only).
  app.use('/api/admin/claude-metrics', authorize('admin'), claudeMetricsRoutes);
}
