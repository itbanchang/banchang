...
const rcS = code.indexOf('{/* ━━━━━ 💰 Financial Strategic Root-Cause Analysis (Unified) ━━━━━ */}');
const hsS = code.indexOf('{/* ━━━ Hero KPI Strip — Professional Design ━━━ */}');
if (rcS > -1 && hsS > -1) {
    code = code.substring(0, rcS) + code.substring(hsS);
}

const fhiS = code.indexOf('{/* ━━━━━ FHI + Professional Finance Analytics Panel ━━━━━ */}');
const prS = code.indexOf('<div className="glass-card" style={{ padding: \\'1rem 1.25rem\\', borderLeft: \\'3px solid #f59e0b\\' }}>');
if (fhiS > -1 && prS > -1) {
...
`;
    // prS index points to the start of the <div className="glass-card" ... prop.
    // So we just add the replacement there to override it and its inner contents up to a point, or just inject this and remove the other part.
    // wait I actually want to replace the whole middle section with `aiPanel`.
    code = code.substring(0, fhiS) + aiPanel + code.substring(code.indexOf('<div className="glass-card" style={{ padding: \\'1rem 1.25rem\\', borderLeft: \\'3px solid #f59e0b\\' }}>', fhiS));
}
