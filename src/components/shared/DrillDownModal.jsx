...
// Professional Analytics Detail View — with Pagination
// ============================================================
...
function usePagination(rows) {
  ...
  const sliced     = (rows || []).slice((safePage - 1) * PAGE_SIZE, safePage * PAGE_SIZE);

  // Reset to page 1 when data changes
  useEffect(() => { setPage(1); }, [rows]);

  return { page: safePage, totalPages, sliced, setPage, total: rows?.length || 0 };
}
...
function PageBtn({ label, disabled, onClick }) {
  return (
    <button
      ...
      disabled={disabled}
      style={{
        padding: '4px 10px', borderRadius: '6px', border: '1px solid var(--md-border)',
        background: disabled ? 'var(--md-bg)' : 'var(--md-surface)',
        color: disabled ? 'var(--md-text-tertiary)' : 'var(--md-text-primary)',
...
const PaginatedTable = React.memo(function PaginatedTable({ headers, rows, renderCell }) {
  const { page, totalPages, sliced, setPage, total } = usePagination(rows);
  ...
  return (
    <div>
      <div style={{ overflowX: 'auto', borderRadius: '12px', border: '1px solid var(--md-border)' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 'var(--fs-sm)' }}>
            ...
            <tr>
              {headers.map((h, i) => (
                <th key={i} style={{
                  ...
                  fontWeight: 800, color: 'var(--md-text-tertiary)',
                  borderBottom: '1px solid var(--md-border)'
                }}>
                  {h}
                </th>
          ...
          <tbody>
            {sliced.length === 0 ? (
              <tr>
                <td colSpan={headers.length} style={{ padding: '2rem', textAlign: 'center', color: 'var(--md-text-tertiary)' }}>
                  ไม่มีข้อมูล
                </td>
              </tr>
            ) : sliced.map((row, i) => (
              <tr key={i} style={{ borderBottom: i === sliced.length - 1 ? 'none' : '1px solid var(--md-divider)' }}>
                {headers.map((_, j) => (
      ...
      <PaginationBar page={paginationInfo.page} totalPages={paginationInfo.totalPages} total={paginationInfo.total} setPage={setPage} />
    </div>
  );
});
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <PaginatedTable
              ...
              rows={data.byDept || []}
              renderCell={(row, j) => j === 2 ? Number(row.revenue).toLocaleString() : row[j === 0 ? 'name' : 'visits']}
            />
          </div>
        </div>
      );
    ...
    default:
      return (
        <div style={{ padding: '2rem', textAlign: 'center' }}>
          ...
          <pre style={{ background: 'var(--md-bg)', padding: '1rem', borderRadius: '12px', fontSize: '11px', textAlign: 'left', overflow: 'auto' }}>
...
function InfoCard({ label, value, color }) {
  return (
    <div style={{ padding: '1rem', borderRadius: '16px', background: `${color}08`, border: `1px solid ${color}20` }}>
      <p style={{ fontSize: '11px', fontWeight: 800, color: 'var(--md-text-tertiary)', textTransform: 'uppercase', marginBottom: '4px' }}>{label}</p>
